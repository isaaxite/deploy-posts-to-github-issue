...
import { isArray, isDataObject, isExecutableFn, isNonEmptyAbsolutePath, isNonEmptyAbsolutePathItemArray, isNonEmptyString, setLinerInterval } from './utils/index.js';
...
export class AssetPublisher {
  ...
  constructor(param) {
    if (!isDataObject(param)) {
      throw new Error('Constructor param muse be object');
    }

    // TODO: push to reposibility of sep by link_prefix preferentially.
  ...
  #setSimpleGitOptAndBaseDirBy({ simpleGitOpt }) {
    if (!simpleGitOpt) {
      this.#simpleGitOpt = DEF_SIMPLE_GIT_OPT;
  ...
  #setAssetRecords(assetRecords) {
    if (!assetRecords) {
      throw new Error('Must be provide assetRecords');
    }

    if (!isArray(assetRecords)) {
      throw new Error('Constructor param assetRecords must be Array');
    }

    for (const it of assetRecords) {
      if (!isNonEmptyAbsolutePath(it.postpath)) {
        throw new Error('assetRecords[].postpath must be non-empty absolute path');
      }

      if (!isNonEmptyAbsolutePathItemArray(it.assetpaths)) {
        throw new Error('assetRecords[].assetpaths must be Array<AbsolutePath>');
      }
    }
  ...
  #setPushHooks(pushHooks) {
    ...
    for (const key of Object.keys(this.#pushHooks)) {
      if (isExecutableFn(pushHooks[key])) {
  ...
  async #resetToPostAndAssetsCommit(resetMode) {
    ...
    if (this.#postAndAssetsCommitId) {
      ret = await this.#git.reset(resetMode, [this.#postAndAssetsCommitId]);
    }
  ...
  async #delBackupBranch() {
    const git = this.#git;
    return await git.deleteLocalBranch(`backup_${this.#uniqueKey}`, true);
  }
  ...
  async #pushPostAndAssets({
    ...
    return new Promise((resolve, reject) => {
      ...
      //   console.info(`git push stderr: ${data}`);
      // });
      ...
      child.on('close', (code, signal) => {
        ...
        return reject(new Error('Publish fail'));
      });
    });
  }
  ...
  async #pushPostAndAssetsWithHint() {
    ...
    const total = 100;
    const bar = new ProgressBar(chalk.greenBright('✓ Pushing ') + '[:bar] :percent', {
      total,
      clear: true,
      width
    });

    const ref = setLinerInterval(() => {
      bar.tick(Math.ceil(total / width));
    }, {
      cnt: width - 1,
      time: 6 * 1000,
      interval: 10
    ...
    try {
      await this.#pushPostAndAssets({ timeout });
      ref.clearLinerInterval();
      bar.update(100);
      hinter.sucMsg('Publish posts and corresponding assets');
    } catch (error) {
      ref.clearLinerInterval();
      bar.update(100);
      throw error;
  ...
  async #push() {
    ...
    this.#addRecoverTask({
      ...
      task: this.#resetToStashCommit.bind(this, ResetMode.HARD)
    });
