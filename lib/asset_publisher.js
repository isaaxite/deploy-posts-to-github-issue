...
export class AssetPublisher {
   ...
   * @property {number} [pushPostAndAssetsTimeout] - only for test
   * 
  ...
  constructor(param) {
    ...
    this.#pushPostAndAssetsTimeout = pushPostAndAssetsTimeout;
  }
  ...
  async #setBackupBranchWithHint() {
    return await this.#hintWraper({
      ...
      callback: async () => {
        await this.#pushHooks.beforeSetBackupBranch();
  ...
  async #pushPostAndAssets({
    ...
    await this.#pushHooks.pushingPostAndAssets({ child });
  ...
  async #pushPostAndAssetsWithHint() {
    const width = 34;
    const timeout = this.#pushPostAndAssetsTimeout || 20 * 1000;
    const total = 100;
