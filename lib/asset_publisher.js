...
import { isArray, isDataObject, isExecutableFn, isNonEmptyString, isNonEmptyStringItemArray } from './utils/index.js';
...
export class AssetPublisher {
  ...
  constructor(param) {
    ...
    this.#git = simpleGit(simpleGitOpt || {});
  ...
  #setAssetRecords(assetRecords) {
    ...
    for (const it of assetRecords) {
      if (!isNonEmptyString(it.postpath)) {
        throw new Error('assetRecords[].postpath must be non-empty string');
      }

      if (!isNonEmptyStringItemArray(it.assetpaths)) {
        throw new Error('assetRecords[].assetpaths must be Array<string>');
      }
    }
