import fs from 'fs';
import { throwFileExistErr, trowCtorParamDataObjectErr, trowDataObjectErr, trowNonEmptyStringErr } from "./utils/error.js";
...
export class MdFrontmatter {
  ...
  constructor(param) {
    ...
    if (markdownTxt) {
      if (!isNonEmptyString(markdownTxt)) {
        trowNonEmptyStringErr('markdownTxt');
      }
      this.#srcMarkdownTxt = markdownTxt;
    } else if (filepath) {
      if (!isNonEmptyString(filepath)) {
        trowNonEmptyStringErr('filepath');
      }
  ...
  genFrontmatterWithFence(frontmatterData) {
    if (!isDataObject(frontmatterData)) {
      trowDataObjectErr('frontmatterData');
    }
