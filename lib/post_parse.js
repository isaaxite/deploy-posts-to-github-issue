...
import {
  DEF_LINK_TYPE_LIST, LINK_OPT_TYPES, FRONTMATTER, EMPTY_CONTENT_AST, DEF_BACK_TO_TOP,
...
export class PostParse {
  ...
  getFormatedMarkdown({ ast, hide_frontmatter } = {}) {
    ...
    const isEnableBack2top = () => {
      ...
      return !!this.#conf.back2top?.enable;
    };
    ...
    if (isEnableBack2top() && lastAst.children?.length) {
      insterBackToTop();
    }
